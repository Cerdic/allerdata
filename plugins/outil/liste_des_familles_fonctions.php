...
function familles_moleculaires($p) {
	...
	$query = "SELECT DISTINCT 
		...
		FROM ((($vue AS tbl_items
			...
			INNER JOIN $vue AS tbl_items_1 ON tbl_est_dans_1.est_dans_id_item = tbl_items_1.id_item)
	...
	while ($row = sql_fetch($res)){
		...
		if (!isset($temp[$id_item])) $temp[$id_item] = array('card' =>0, 'pos' =>$pos, 'est_dans' => array(), 'nom' => $nom, 'id_item' => $id_item, 'mt' => $row['id_minitexte']?$row['id_minitexte']:'');
