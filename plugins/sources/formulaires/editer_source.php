...
function formulaires_editer_source_verifier_dist($id_item='new', $id_parent=0, $retour='', $lier=0, $config_fonc='', $row=array(), $hidden=''){
	...
	if ($rows = sql_allfetsel("id_item,nom",'tbl_items',
	  "id_type_item=4 AND nom=".sql_quote(_request('nom'))." AND NOT(id_item=".intval($id_item).")")
	  ){
		$liens = array();
		foreach($rows as $row){
			$liens[] = "<a href='".generer_url_ecrire('allerdata','page=sources&id_item='.$row['id_item'])."' title='#".$row['id_item']."'>".$row['nom']."</a>";
		}
		$liens = implode(", ",$liens);
		$erreurs['nom'] = _T("editer_source:item_deja_existant_avec_meme_nom").$liens;
	}
	return $erreurs;
}
