...
function traduit_pays($texte, $lang=null){
	...
	$texte = preg_replace_callback(",\w[\w\s-]+\w,u",'pays_trad_array',translitteration($texte));
	return $texte;
}
function pays_trad_array($t){
	return pays_trad_from_fr(trim($t[0]));
}

function pays_trad_from_fr($pays,$lang='en'){
	static $trad_pays = null;
	if (!$pays) return $pays;
	if (!$trad_pays){
		$t=array(
		...
		foreach($t as $i){
			$p = extraire_multi($i['nom'],'fr');
			$trad_pays[$p]['en'] = extraire_multi($i['nom'],'en');
			$pt = translitteration($p);
			$trad_pays[strtolower($pt)] = $trad_pays[$p];
		}

		// alias
		$trad_pays['usa'] = $trad_pays['États-Unis'];
		$trad_pays['grande bretagne'] = $trad_pays['Royaume-Uni'];
		$trad_pays['coree']['en'] = 'Korea';
		$trad_pays['non precise']['en'] = 'Unknown';
		$trad_pays['?']['en'] = '?';
		$trad_pays['canaries']['en'] = 'Canaria';
	}

	if (isset($trad_pays[$pays][$lang]))
		return $trad_pays[$pays][$lang];

	$pt = strtolower(translitteration($pays));
	if (isset($trad_pays[$pt][$lang]))
		return $trad_pays[$pt][$lang];
